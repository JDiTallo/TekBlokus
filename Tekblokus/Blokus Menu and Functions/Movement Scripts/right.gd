	...
	if draggable:
		...
		if Input.is_action_pressed("click"):
			...
			if is_inside_dropable:
				...
				if w:
					global.w_moves.append("move_right")
					#up_position = global.w_moves.size()
					
				if a:
					global.a_moves.append("move_right")
					
				if s:
					global.s_moves.append("move_right")
					
				if d:
					global.d_moves.append("move_right")

			if not is_inside_dropable:
				#global.w_moves.resize(0)
				global.w_moves.erase("move_right")
				global.a_moves.erase("move_right")
				global.s_moves.erase("move_right")
				global.d_moves.erase("move_right")
